...
func (attendanceApi *AttendanceApi) GetAttendanceHistory(c *gin.Context) {
	var pageInfo checkinsReq.AttendanceSearchHistory
	err := c.ShouldBindQuery(&pageInfo)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	list, total, err := attendanceService.GetAttendanceHistory(pageInfo)
	if err != nil {
		global.GVA_LOG.Error("lấy thất bại!", zap.Error(err))
		response.FailWithMessage("lấy thất bại:"+err.Error(), c)
		return
...
func (attendanceApi *AttendanceApi) StatsScatterPlot(c *gin.Context) {
	var stats checkinsReq.StatsByInfoRequest
	err := c.ShouldBindJSON(&stats)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
	...
	res, err := attendanceService.StatsScatterPlot(stats)
	if err != nil {
		global.GVA_LOG.Error("Không lấy được dữ liệu!", zap.Error(err))
		response.FailWithMessage("Không lấy được dữ liệu:"+err.Error(), c)
		return
