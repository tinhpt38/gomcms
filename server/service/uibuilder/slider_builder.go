...
func (sliderBuilderService *SliderBuilderService) GetPublicSlider() (slider uibuilder.SliderBuilder, err error) {

	db := global.GVA_DB.Model(&uibuilder.SliderBuilder{})
	err = db.Where("is_show = 1").Order("created_at desc").First(&slider).Error
	return slider, err
}
