<template>
  <div>
    ...
    <el-table :data="tableData" style="width: 100%" border>
      ...
      <el-table-column prop="allowRestrictIp" label="IP khu vực">
        <template #default="scope">
          {{ scope.row.allowRestrictIp ? 'Có' : 'Không' }}
        </template>
      </el-table-column>
    ...
    <el-drawer v-model="dialogFormVisible" destroy-on-close size="800" :show-close="false" :before-close="closeDialog">
      ...
      <el-form ref="elFormRef" :model="formData" label-position="top" :rules="rules" label-width="80px">
        <el-form-item label="Tên khu vực:" prop="areaId">
          <el-select v-model="formData.areaId" placeholder="Chọn khu vực" clearable filterable @change="onSelectChange">
        ...
        <el-form-item label="Giới hạn IP theo khu vực:" prop="allowRestrictIp">
          <el-switch v-model="formData.allowRestrictIp" active-text="Có" inactive-text="Không" />
        </el-form-item>
      </el-form>
    </el-drawer>
  </div>
