                        ...
                        <div class="mx-auto max-w-2xl gap-x-14 lg:mx-0 lg:flex lg:max-w-none lg:items-center">
                            <div class="w-full max-w-xl lg:shrink-0 xl:max-w-2xl">
                                ...
                                <div v-if="conditionData.length > 0" class="px-4 sm:px-0">
                                    <h3 class="text-base font-semibold leading-7 text-gray-900">Danh sách cần điểm danh
                                    </h3>
                                    <dl class="divide-y divide-gray-100">
                                        <div v-for="(item, key) in conditionData"
                                            class="px-2 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                            <dt class="text-sm font-medium leading-6 text-gray-900">Lần thứ {{ key + 1
                                                }}
                                            </dt>
                                            <dd class="my-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
                                                {{ conditionString(item) }}</dd>
                                            <el-tag effect="dark" type="success" v-if="item.isPass">Đã điểm
                                                danh</el-tag>
                                            <el-tag effect="dart" type="danger" v-if="!item.isPass">Chưa điểm
                                                danh</el-tag>
                                        </div>
                                    </dl>
                                </div>
