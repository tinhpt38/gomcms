<template>
  <div>
    <div class="sticky top-0.5 z-10">
      <el-input v-model="filterText" class="w-3/5" placeholder="Lọc" />
      <el-button class="float-right" type="primary" @click="relation">Xác nhận</el-button>
    </div>
            ...
            <span class="custom-tree-node">
              ...
              <span v-if="node.checked">
                <el-button type="primary" link :style="{ color: row.defaultRouter === data.name ? '#E6A23C' : '#85ce61' }"
                  ...
                  {{ row.defaultRouter === data.name ? "Trang chủ" : "Đặt làm trang chủ" }}
                </el-button>
              </span>
    ...
    <el-dialog v-model="btnVisible" title="Phân quyền nút" destroy-on-close>
      <el-table ref="btnTableRef" :data="btnData" row-key="ID" @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="55" />
        <el-table-column label="Tên nút" prop="name" />
        <el-table-column label="Ghi chú nút" prop="desc" />
      </el-table>
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="closeDialog">Hủy bỏ</el-button>
          <el-button type="primary" @click="enterDialog">Xác nhận</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
