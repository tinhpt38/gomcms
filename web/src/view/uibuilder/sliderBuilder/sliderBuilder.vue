  ...
  <div>
        ...
        <el-table-column label="Hình ảnh" prop="images" width="200">
          <template #default="scope">
            <div class="multiple-img-box">
              <el-image preview-teleported v-for="(item, index) in scope.row.images" :key="index"
        ...
        <el-table-column align="left" label="Hành động" fixed="right" min-width="240">
          <template #default="scope">
            ...
            <el-button type="primary" link icon="edit" class="table-button"
              @click="updateSliderBuilderFunc(scope.row)">Chỉnh sửa</el-button>
    ...
    <el-drawer destroy-on-close size="800" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
      ...
      <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
        <el-form-item label="Title:" prop="title">
          <el-input v-model="formData.title" placeholder="Nhập tiêu đề"></el-input>
        </el-form-item>
        <el-form-item label="Sub Title:" prop="subTitle">
          <el-input v-model="formData.subTitle" placeholder="Nhập tiêu đề"></el-input>
        </el-form-item>
        <el-form-item label="Hình ảnh:" prop="images">
          <SelectImage multiple v-model="formData.images" file-type="image" />
        </el-form-item>
        <el-form-item label="Content:" prop="content">
          <el-input v-model="formData.content" type="textarea" placeholder="Nhập nội dung"></el-input>
        </el-form-item>
    ...
    <el-drawer destroy-on-close size="800" v-model="detailShow" :show-close="true" :before-close="closeDetailShow">
      <el-descriptions column="1" border>
        <el-descriptions-item label="Hình ảnh">
          <el-image style="width: 50px; height: 50px; margin-right: 10px"
            :preview-src-list="returnArrImg(detailFrom.images)" :initial-index="index"
            v-for="(item, index) in detailFrom.images" :key="index" :src="getUrl(item)" fit="cover" />
        </el-descriptions-item>
        <el-descriptions-item label="Title">{{ detailFrom.title }}</el-descriptions-item>
...
const getTableData = async () => {
  const table = await getSliderBuilderList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async () => {
  ElMessageBox.confirm('Bạn có chắc muốn xóa không?', 'Cảnh báo', {
    ...
    const res = await deleteSliderBuilderByIds({ IDs })
    if (res.code === 0) {
      ElMessage({
        type: 'success',
...
// Đánh dấu hành động (thêm hoặc sửa) trong hộp thoại
const type = ref('')

// Cập nhật hàng
const updateSliderBuilderFunc = async (row) => {
  const res = await findSliderBuilder({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
...
// Xóa hàng
const deleteSliderBuilderFunc = async (row) => {
  const res = await deleteSliderBuilder({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: 'Xóa thành công'
